{
  ...
  let newline lexbuf =
    lexbuf.lex_curr_p <- { (lexeme_end_p lexbuf) with
      pos_lnum = (lexeme_end_p lexbuf).pos_lnum + 1; (* line number increased by 1 *)
...
let (symbol_table : (string, Parser.token) Hashtbl.t) = Hashtbl.create 1024
  ...
  let create_token lexbuf =  (* outputs the corresponding token associated with the input *)
    let str = lexeme lexbuf in  (* lexeme lexbuf := return the string matched by the regular expression*)
    try (Hashtbl.find symbol_table str)  
    with _ -> IDENT str (* if not found, create an identification*)
